...
enum class PresentationErrorCode(
    ...
    PRESENTATION_MONG_NAME("이름은 6자 제한", true),

    // Battle
    PRESENTATION_BATTLE_NOT_ENOUGH_PAY_POINT("페이 포인트 부족", true),

    // Slot
    PRESENTATION_MANAGEMENT_NOT_PICK_SLOT("선택된 슬롯이 없음", true),

    // Billing
    PRESENTATION_USER_BILLING_CONNECT("인앱 결제 시스템 연결 실패", false),
    PRESENTATION_USER_BILLING_NOT_SUPPORT("결제 미지원 기기", true),
