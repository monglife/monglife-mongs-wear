...
### ⚠️ 원본 DTO 를 믿지 말고 실제 페이로드를 볼 것

Android `MatchPlayerEventDto` 에는 `mongId` 가 있지만 **서버는 보내지 않는다.**
DTO 만 보고 옮겼더니 `Match` 디코딩이 통째로 실패해 **라운드가 한 번도 도착하지 않았다** —
화면은 "입장 중"에서 조용히 멈추고, 오류 배너조차 뜨지 않는다.

그래서 `BattleService` 의 해석 실패 로그에 **오류 원인과 원문을 함께 남긴다**(원문은 Debug 전용).
그게 없으면 "메시지를 못 받는 것"과 "받았는데 못 읽는 것"을 구별할 수 없다.

```bash
xcrun simctl spawn <UDID> log stream --level debug \
  --predicate 'subsystem == "com.monglife.mongs.wear.ios"' --style compact
```

### 혼자 큐에 들어가면 서버가 봇을 붙인다

기기 두 대가 없어도 배틀 전체 흐름을 볼 수 있다 —
잠시 기다리면 `안씻은 별별몽 봇` 같은 상대가 매칭된다.

### ⚠️ 브로커가 와일드카드 구독을 주지 않는다

`mongs-dev/battle/#` 로 구독하면 **조용히 아무것도 오지 않는다**(자가 발행도 못 받는다).
MQTT 트래픽을 엿보려면 **정확한 토픽**으로 걸어야 하고, 한 번에 너무 많이 걸면
일부가 조용히 거부된다. 디버깅할 때는 위의 앱 로그가 더 확실하다.
