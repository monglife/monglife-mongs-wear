package com.mongs.wear.presentation.pages.exchange.starPoint
...
import com.mongs.wear.presentation.pages.exchange.starPoint.ExchangeStarPointViewModel.UiState
...
fun ExchangeStarPointView(
    ...
    val mongVo = exchangeStarPointViewModel.mongVo.observeAsState()
    val ratio = remember { mutableIntStateOf(0) }
    val totalStarPoint = exchangeStarPointViewModel.starPoint.observeAsState(0)
    ...
    Box {
        ...
        } else if(exchangeStarPointViewModel.uiState.exchangeStarPointDialog) {
            ConfirmAndCancelDialog(
                text = "$${chargePayPoint.value}\n환전하시겠습니까?",
                confirm = {
                    mongVo.value?.let {
                        exchangeStarPointViewModel.exchangeStarPoint(
                            mongId = it.mongId,
                ...
                cancel = { exchangeStarPointViewModel.uiState.exchangeStarPointDialog = false }
            )
        } else {
            ExchangeStarPointContent(
                ...
                uiState = exchangeStarPointViewModel.uiState,
...
@Composable
private fun ExchangeStarPointLoadingBar(
    modifier: Modifier = Modifier.zIndex(0f),
) {
    Box(
        contentAlignment = Alignment.Center,
